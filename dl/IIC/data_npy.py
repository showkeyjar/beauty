...
def load(data_set_name, **kwargs):
    ...
    data_files = glob.glob(data_path + "*.npy")
    # 参考: https://stackoverflow.com/questions/48889482/feeding-npy-numpy-files-into-tensorflow-data-pipeline
    dataset = tf.data.Dataset.from_tensor_slices(data_files)
    ds = dataset.map(lambda item: tuple(tf.py_func(read_npy_file, [item], [tf.float32,])))
