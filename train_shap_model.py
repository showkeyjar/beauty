...
def shap_save_to_disk(explainer, features, seed):
    print("Saving SHAP to .h5 file...")
    h5_file = 'model/shap_model.h5'
    ...
    shap_val_df = pd.DataFrame(shap_values)  # this step is unnecessary, could just save np arrays directly, but the df have some advantages and I don't mind reconverting upon loading
    shap_feat_df = pd.DataFrame(features_shap)
    # define what goes in the first row with "d"
    d = [[explainer.target, explainer.name_for_figs, explainer.shap_expected, explainer.class_thresh]]
    exp_df = pd.DataFrame(
...
if __name__ == "__main__":
    explainer = shap.KernelExplainer(model.predict, shap.kmeans(df_input, 100))

    # joblib.dump(explainer, 'model/explainer.pkl', compress=('bz2', 9))
    with open('model/explainer_1.pkl', 'wb') as f:
