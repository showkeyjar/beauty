...
def remove_duplicates():
    ...
    encodings = phasher.encode_images(image_dir='data/face')
    # 对已编码图像寻找重复图像
    duplicates = phasher.find_duplicates(encoding_map=encodings)
    logger.debug("find duplicates:" + str(len(duplicates)))
    print(duplicates[list(duplicates.keys())[0]])
    dup_list = dict_to_list(duplicates)
    new_img = 0
    # 删除除第1条记录以外的其他文件，要与知识库建立联系，不能随便删除
    for dup in dup_list:
        # 将第一张图片拷贝到目标目录即可
        try:
            shutil.copy("data/face/" + dup[0], "data/unique_face/" + dup[0])
            logger.debug("copy file " + dup[0])
        except Exception as e:
            logger.warning(e)
        # 检查是否已收录
