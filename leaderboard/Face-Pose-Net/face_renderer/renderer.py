...
def warpImg(img, t_height, t_width, prj, idx):
    ...
    # fixed by: https://github.com/iacopomasi/face_specific_augm/issues/3#issuecomment-341615461
    if prj.size != 0:
        xs = np.squeeze(np.asarray(prj[0, :])).reshape(t_height, t_width).astype(np.float32)
        ys = np.squeeze(np.asarray(prj[1, :])).reshape(t_height, t_width).astype(np.float32)
        pixels = cv2.remap(img, xs, ys,  cv2.INTER_CUBIC)
...
def HandleBackground(out_proj, face_proj_in, img, opts):
    if out_proj.size != 0:
        ...
        widthX = lenn[1]
        heightY = lenn[0]
        thWidth = face_proj_in[0, :].min()/img.shape[1] * \
            opts.getfloat('renderer', 'scaleFaceX')
        idxOveral = np.nonzero(np.squeeze(
            np.asarray(out_proj[0, :])) > thWidth)[0]
        if idxOveral.size != 0:
            out_proj[0, idxOveral] = out_proj[0, idxOveral] / \
                out_proj[0, idxOveral].max()*thWidth
        ...
        out_proj = UnnormalizePoints(
            out_proj, np.matrix([img.shape[1], img.shape[0]]))
    return out_proj
...
def render(img, proj_matrix, ref_U, eyemask, facemask, opts):
    ...
    temp_proj = proj_matrix * \
        np.vstack((threedee, np.ones((1, threedee.shape[1]))))
    ...
    bg_mask = np.setdiff1d(
        np.arange(0, ref_U.shape[0]*ref_U.shape[1]), facemask[:, 0])
    ...
    if opts.getboolean('renderer', 'background'):
        frontal_raw = warpImg(
            img, ref_U.shape[0], ref_U.shape[1], prj_jnt, ind_jnt)
    else:
        frontal_raw = warpImg(
            img, ref_U.shape[0], ref_U.shape[1], face_proj, facemask[:, 0])

    # Apply soft-sym if needed
    frontal_sym, sym_weight = mysoftSymmetry(
        img, frontal_raw, ref_U, in_proj, ind_frontal, bg_mask, facemask[:, 0], eyemask, opts)
...
def mysoftSymmetry(img, frontal_raw, ref_U, in_proj,
    ...
    # one side is ocluded
    if eyemask is not None and opts.getboolean('renderer', 'symmetry'):
        # Soft Symmetry param
        ...
        synth_frontal_acc = synth_frontal_acc.reshape(
            (ref_U.shape[0], ref_U.shape[1]), order='F')
        synth_frontal_acc = cv2.GaussianBlur(
            synth_frontal_acc, (ksize_acc, ksize_acc), 30., borderType=cv2.BORDER_REPLICATE)
        ...
        synth_frontal_acc[facemask] = (
            synth_frontal_acc[facemask] - minacc)/(maxacc-minacc)
        ...
        synth_frontal_acc = np.tile(synth_frontal_acc.reshape(
            ref_U.shape[0], ref_U.shape[1], 1), (1, 1, 3))
        # Flipping
        frontal_flip = frontal_raw.copy()
        print("midcolumn:", midcolumn)
        frontal_flip[:, 0:midcolumn, :] = np.fliplr(frontal_flip)[
            :, 0:midcolumn, :]
        frontal_sym = np.multiply(
            frontal_raw, 1.-synth_frontal_acc) + np.multiply(frontal_flip, synth_frontal_acc)
